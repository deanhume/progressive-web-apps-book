...
function streamArticle() {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const bodyData = fetch('./data/data-1.html').catch(() => new Response("Failed, soz"));
      ...
      startFetch
        ...
        .then(() => endFetch)
        .then(endFetch => pushStream(response.body))
...
self.addEventListener('fetch', function (event) {
  ...
  if (/article.html/.test(event.request.url)) {
    event.respondWith(streamArticle());
  }
...
//https://jakearchibald.github.io/isserviceworkerready/demos/simple-stream/sw.js
